...
function block(post) {
    ...
    root.innerHTML = `<div style="display:flex; flex-direction: column; height: 600px; width: 500px; background-color: white; font-family: 'Lato', sans-serif; border-radius: 15px; justify-content: center; align-items: center;margin-bottom: 15px;" id="${post.id}">  <link href="https://fonts.googleapis.com/css?family=Lato" rel="stylesheet"> <div id="factcat-popup-${post.id}" style="transition-duration: 1s; display: none; position: absolute; z-index: 9999; background-color: #5BD9BC; border-radius: 15px; height: 300px; width: 250px; padding: 20px;font-family: 'Lato', sans-serif; color:white;"> <h1>Analysis</h1> <label>Credibility</label> <br><progress value="0.5"></progress> <br><label>Language</label> <br><progress value="0.5"></progress> <br><label>Validity</label> <br><progress value="0.5"></progress> <br><button style="float:right;border: none; background-color: white; color: black; height: 28px; font-size: 14px;font-weight: 700; border-radius: 10px;" onclick="document.getElementById('factcat-popup-${post.id}').style.display='none'">Close</button> </div><img class="cat" src="https://image.ibb.co/njL3bo/cat.png" alt="cat" style="padding-bottom:100px; position: absolute; width: 250px; object-position: center; object-fit: fill;"> <div style="flex: 9;"></div><div style="flex: 5; width:100%; padding-top: 10%; background-color: #f5f5f5f5; text-align: center;"> <h1 style="font-size: 20px; font-weight: 700; ">Meow!</h1> <h2>This news may be unreliable!</h2> </div><div on class="factcat-footer" style="float: bottom; width:100%; flex: 1; color: white; background-color: #5BD9BC;"> <button onclick="document.getElementById('factcat-popup-${post.id}').style.display='block'" style="margin: 10px; font-size:16px; float: left; border: none; background-color: #5BD9BC; color: white;">Why is there a fact cat?</button> <p style="margin: 10px; font-size:16px; float: right;">FACTCAT</p></div><style>#factcat-popup>progress{margin-bottom: 10px; border: none; width: 80%;}#factcat-popup>label{margin-top: 10px;}</style></div>`;
